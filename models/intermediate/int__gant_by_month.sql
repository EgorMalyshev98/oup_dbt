...
with recursive
    cte as (
        select gant_index, code, start_date, end_date, project_type, "object", "Calen"
    ...
    cte_3_ as (  -- noqa: ST03
        select
            ...
            case
                when
                    extract(month from cte_1.new_start_date)
                    = extract(month from rsce."Start")
                then
                    /* поиск месяца операции в списке исключений*/
                    case
                        when
                            cte_1.new_start_date
                            between (
                                rsce."Start" + (
                                    date_trunc('year', cte_1.new_start_date)
                                    - date_trunc('year', rsce."Start")
                                )
                            ) and (
                                rsce."Fin" + (
                                    date_trunc('year', cte_1.new_end_date)
                                    - date_trunc('year', rsce."Fin")
                                )
                            )
                        then
                            (
            ...
            case
                when
                    extract(month from cte_1.new_end_date)
                    = extract(month from rsce."Fin")
                then
                    /* поиск месяца операции в списке исключений*/
                    case
                        when
                            ...
                            and extract(month from cte_1.new_end_date)
                            = extract(month from cte_1.new_start_date)
                        then
                            (
                                rsce."Fin_true" + (
        ...
        from cte_1
        left join
            {{ source("spider", "raw_spider__calen_except") }} as rsce  -- raw_spider__calen_except rsce
            on extract(month from cte_1.new_start_date)
            = extract(month from rsce."Start")
            and cte_1."Calen" = rsce."Calen_Code"
        where
            -- не рассматривать исключения календарей SR_10 и SR_20
            coalesce(
                cte_1."Calen" like 'SR_%'
                and extract(month from cte_1.new_start_date) in (1, 2, 3, 12),
                false
            )
            is false  -- noqa: LT05
    ),
    ...
    cte_2 as (
        select
            ...
            count(*) over (partition by code, gant_index) as row_oper_count
...
select
    ...
    round(
        (weight * (coalesce(r."c_pln_SMRSsI", 0) + coalesce(r."c_pln_SMRSpI", 0)))
    ) as "SMRFull",
    round(
        (
            weight * (
