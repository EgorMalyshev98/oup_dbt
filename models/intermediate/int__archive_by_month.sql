...
tmp2 as (
    select
        ...
        case
            ...
            when start_date = max(start_date) over (partition by code, archive_index)
                and date_trunc('month', start_date) = date_trunc('month', end_date)
                then date_part('day', end_date)
            when start_date = min(start_date) over (partition by code, archive_index)
                then
                    case
                        when date_part(
                                'day',
                                (date_trunc('month', start_date) + interval '1 month') - date_trunc('day', start_date)
                            )
                            = 0
                            then 1
                        else date_part(
                    ...
                    'day', (date_trunc('month', start_date) + interval '1 MONTH' - date_trunc('month', start_date))
                )
...
final as (
    ...
    inner join {{ source('spider', 'raw_spider__archive') }} as r
