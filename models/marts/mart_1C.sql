...
with t1 as (
    ...
    left join {{ source('dicts', 'dict__1c_objects_to_spider') }} as don on czh.territory_value = don.territory_value
...
select -- noqa: ST06
    ...
    abs(
        round((t1."Фактическая трудоемкость" / t1."Нормативная трудоемкость" * 100 - 100), 2)
...
left join
    ...
    on replace(cw.structure_unique_code, ' ', '') = rsg."f_CodeIdent" and t1."object" = rsg."object"
--соединение со сводной таблицей контрактных позиций
left join
    {{ ref('int__vdc_by_objects') }} as vdc
    on rsg."Num_Con" = vdc."Шифр единичной расценки" and t1."object" = vdc."object"
--соединение с таблицей 1с ЖУФВР
left join {{ source('1c', '1_c__zhufvr') }} as czh on cw.zhufvr_id = czh.link
--соединение с таблицей 
left join
    {{ source('dicts', 'dict__spider_key_res') }} as skr
    on t1."Тип ЕНРП" = skr."f_opertype" and t1."Код ресурса" = skr."rescode"
