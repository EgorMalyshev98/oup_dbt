...
with 
-- tbl1 - объединение (union all) 2-х таблиц <Техника рабочие> и <Нормативная трудоемкость>, 
...
where
	...
	NOT EXISTS (SELECT 1
				from "oup"."1c"."1_c__norm_workload" nwd
...
tbl2 as (
select 
	sum(tbl1."Фактическая трудоемкость") as "hours_from_mart"
from tbl1),
-- tbl3 -- сумма фактической трудоемкости (фактических часов) в таблице <Техника рабочие>  
tbl3 as (
select 
	sum(ctw.hours) as "hours_from_tchnq_wrkrs"
from {{ source('1c', '1_c__technique_workers') }} as ctw
{# oup."1c"."1_c__technique_workers" as ctw  #}
)
...
select 
	round((tbl2."hours_from_mart" - tbl3."hours_from_tchnq_wrkrs") 
		/ tbl3."hours_from_tchnq_wrkrs" * 100, 2) as "delta_hours" 
-- delta_hours - отклонение суммы часов в витрине <mart__1C> от суммы таблицы <Техника рабочие> в %. 
-- за 100% принята сумма часов из таблицы <Техника рабочие>. 		
from tbl2, tbl3
where 
	round((tbl2."hours_from_mart" - tbl3."hours_from_tchnq_wrkrs") 
		/ tbl3."hours_from_tchnq_wrkrs" * 100, 2) >= 0.1 -- "delta_hours"
-- 0,1% от суммы часов в таблице <Техника рабочие> - принятое значение 
