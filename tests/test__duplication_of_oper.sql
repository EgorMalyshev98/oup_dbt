...
with
    ...
    final_pln as (
        ...
        from t1
        where
            (smr_full_pln != 0 and smr_full_mart != 0)
    ...
    t1f as (
        select
            "OperCode",
            mgabm.code,
            coalesce("c_aac_SMRSsI", 0) as smr_ss_fct,
            coalesce("c_aac_SMRSpI", 0) as smr_sp_fct,
        ...
        inner join
            {{ ref("mart__gant_archive_by_month") }} as mgabm
            on fct."OperCode" = mgabm.code

        where
            fct.project_type = 'проект'
            and mgabm.smr_type = 'факт'
            and fct."ResCode" is not null

        group by mgabm.code, "OperCode", "c_aac_SMRSsI", "c_aac_SMRSpI"
    ),

    final_fct as (
        select *, smr_full_fct - smr_full_mart as delta
        from t1f
        where
            (smr_full_fct != 0 and smr_full_mart != 0)
            and (smr_full_fct - smr_full_mart != 0)
    ),

    fnlf as (
        select *, abs(delta)  -- noqa
        from final_fct
        where abs(delta) > 1
